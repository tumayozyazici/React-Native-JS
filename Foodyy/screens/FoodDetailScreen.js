...
export default function FoodDetailScreen({ route, navigation }) {
  ...
  const isFoodFavorite = favoriteFoodContext.ids.includes(foodId);
  ...
  function changeFavorite(){
    isFoodFavorite ? favoriteFoodContext.removeFavorite(foodId) : favoriteFoodContext.addFavorite(foodId)
  }
    ...
    navigation.setOptions({
      headerRight: () => {
        return (
          ...
          >
            <Ionicons name={isFoodFavorite ? 'star' : 'star-outline'} size={24} color="white" onPress={changeFavorite} />
          </Pressable>
        );
      },
    });
